...
public class GrassField : MonoBehaviour
{
	...
	[SerializeField] private AbstractGrassDrawer abstractGrassDrawer;
	...
	{
		//Field generation
		_startPosition = -_fieldSize / 2.0f + new Vector2(transform.position.x - _fieldSize.x / 2.0f, transform.position.z - _fieldSize.y / 2.0f);
		...
		abstractGrassDrawer.Init(grassEntities, _fieldSize);
	...
	private void UpdateCameraCells()
	{
		if (!PerformCulling)
		{
			abstractGrassDrawer.UpdatePositions(Vector2Int.zero, new Vector2Int(GrassDensity, GrassDensity));
			return;
		}
		...
		var topRightCameraCell = new Vector2Int(
			...
			Mathf.Clamp(Mathf.FloorToInt((topRightCameraCorner.z - _startPosition.y) / _cellSize.y) + 1, 0,
				GrassDensity - 1));
		abstractGrassDrawer.UpdatePositions(bottomLeftCameraCell, topRightCameraCell);
	}
}